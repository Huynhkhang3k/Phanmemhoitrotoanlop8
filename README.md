...
<html lang="vi">
  <head>
    <meta charset="UTF-8" />
    <title>Phần mềm hỗ trợ học tập lớp 8</title>
    ...
    <style>
      ...
      * {
      ...
      /* Sắp xếp 3 ô cạnh AB, BC, AC liền nhau */
      .three-sides {
  ...
  <body>
    <!-- Thanh điều hướng -->
    <div class="topBar">
      <button id="btnCalcMath">
        Máy tính biểu thức và tính phương trình bậc Nhất
      </button>
      <button id="btnCalcTriangle">Máy tính định lý Pythagoer</button>
      <button id="btnCalcMidpoint">
        Máy tính đường Trung Bình của tam giác
      </button>
    </div>

    <!-- 1) Máy tính Toán Học -->
    <div class="container" id="calcMath">
      <h1>Máy tính biểu thức và tính phương trình bậc Nhất</h1>
      <p class="note">
        Lưu ý: Nếu nhập biến số, hãy viết theo đúng dạng <em>ax+b=y</em> (hệ số
        luôn đứng trước biến).<br />
        • Không hỗ trợ giải phương trình bậc 2, 3, 4,...<br />
        • Nếu viết sai thứ tự (ví dụ: <code>x3=3</code> thay vì
        <code>3x=3</code>), máy tính sẽ báo lỗi.<br />
        • Khi nhập biến số, phần giải thích biểu thức số học gốc sẽ bị khóa và
        thay bằng giải thích phương trình bậc 1.
      </p>
    ...
    <div class="container" id="calcMidpoint">
      <h1>Máy tính đường Trung Bình của tam giác</h1>
      <p>
        Nhập giá trị: <strong>AB, BC, AC</strong> (3 cạnh của tam giác). Các giá
        trị còn lại sẽ được tự tính theo ràng buộc: <br />– D là trung điểm AB
        (⇒ AD = BD, AB = 2×AD) <br />– E là trung điểm BC (⇒ BE = EC, BC = 2×BE)
        <br />– DE = AC/2
      </p>
      <!-- Ô nhập 3 cạnh liền nhau -->
      <div class="input-fields three-sides" style="display: grid">
    ...
    <script>
      ...
      function attachKeyListeners() {
        ...
        keys.forEach((key) => {
          ...
          key.addEventListener("click", () => {
            ...
            // Nếu có biến, khóa các nút "xⁿ" và "x mũ -n"
            const hasVar = /[xyzabcdvts]/.test(getExpressionPlainText());
            if (hasVar && (value === "power" || value === "xpowerNeg")) {
              return;
            }
            if (value === "toggleVar") {
      ...
      // Sửa lại hàm parseSide: nếu có biến mà không có số (tức hệ số) trước biến thì báo lỗi
      function parseSide(side) {
        ...
        while ((match = regex.exec(side)) !== null) {
          ...
          // Nếu có biến mà không có số, báo lỗi
          if (variable && numStr === "") {
            error = true;
            break;
          }
        ...
        if (error)
          throw new Error("Biểu thức sai thứ tự: Hệ số phải đứng trước biến.");
      ...
      // Hàm giải thích chi tiết cho biểu thức số học (không có biến)
...
<strong>Kết quả của biểu thức "<em>${expr}</em>" là:</strong> <span style="font-size:1.8em;">${result}</span>.`;
      ...
      // Hàm calculate: xử lý riêng biểu thức không có biến (sử dụng giải thích gốc) và có biến (phương trình bậc 1)
      function calculate() {
        let expression = getExpressionPlainText();
        let hasVar = /[xyzabcdvts]/.test(expression);
        // Nếu không có biến, nếu có dấu "=" loại bỏ để tính toán thông thường
        if (expression.includes("=") && !hasVar) {
          expression = expression.replace("=", "");
        }
        // Nếu có biến -> giải phương trình bậc 1 với giải thích mới và khóa giải thích số học gốc
        if (expression.includes("=") && hasVar) {
          try {
            let solution = solveEquation(expression);
            let [leftExpr, rightExpr] = expression.split("=");
            let left = parseSide(leftExpr);
            let right = parseSide(rightExpr);
            let variable = extractVariable(expression);
            // Tính tổng hệ số của biến ở một vế (A) và tổng số hạng tự do ở vế kia (B)
            let A =
              (left.coefficients[variable] || 0) -
              (right.coefficients[variable] || 0);
            let B = (right.constant || 0) - (left.constant || 0);
            let explanation = `Phương trình được giải: ${variable} = ${solution}<br><br>`;
            explanation += "Giải thích chi tiết các bước:<br>";
            explanation +=
              "- Bước 1: Đưa các hạng tử chứa " +
              variable +
              " về một vế và các số hạng tự do về vế kia (đổi dấu số hạng tự do).<br>";
            explanation +=
              "- Bước 2: Tính tổng hệ số của " +
              variable +
              " ở một vế (gọi là A) và tổng số hạng tự do ở vế kia (gọi là B).<br>";
            explanation += "  • Ví dụ: A = " + A + " và B = " + B + ".<br>";
            explanation +=
              "- Bước 3: Lấy B chia cho A để tìm nghiệm: " +
              variable +
              " = B/A = " +
              solution +
              ".<br><br>";
            explanation += `<strong>Kết quả của biểu thức "<em>${expression}</em>" là:</strong> <span style="font-size:1.8em;">${variable} = ${solution}</span>.`;
            document.getElementById("result").innerText =
              variable + " = " + solution;
            document.getElementById("explanation").innerHTML = explanation;
          } catch (e) {
            document.getElementById("result").innerText = "Lỗi!";
            document.getElementById("explanation").innerText = e.message;
          }
          return;
        }
        // Nếu không có biến: sử dụng tính toán và giải thích gốc cho biểu thức số học
    ...
    <!-- 3) Mã cho Máy Tính Tam Giác Trung Điểm -->
    <script>
      let vals = {
        AB: null,
        AD: null,
